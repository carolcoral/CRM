...
function check(dealtime,courseid,dealprice,ishavenetedu,netedumoney,remituser,idCard,email,address,courseid,banci){
	 ...
	 if(courseid==""||courseid==null){
		 ...
		 flag = false;
	 }else if(courseid == "1" || courseid == "3"){
		if(banci == null || banci == ""){               
		    $('#courseError').html("班级不能为空")
		    flag = false;                  
		}else {
		    $('#courseError').html("")
		}
	...
	if(ishavenetedu=="1"){
	 ...
	 if((idCard == "" || idCard == null) || (email == "" || email == null) || (address == "" || address == null)){
		$('#dealError').html("手机号、身份证号、邮箱、地域不可为空");
		flag = false; 
	 }else {
		$('#dealError').html("");
	 }
...
function adddeal(){
	...
	if(check(dealtime,courseid,dealprice,ishavenetedu,netedumoney,remituser,idCard,email,address,courseid,banci)){
...
function saveDetails(){
	...
	var address = $("#address option:selected").val();
	var banci = $("#banci option:selected").val();
	var otherwise = $("#otherwise").val();
	var isjieye = $("#isjieye option:selected").val();
	var companyAddr = $("#companyAddr").val();
	var department = $('#department').val();	
	
	if(isjieye == "1"){
		if((LCWname == "" || LCWname == null) || (LCWpassword == "" || LCWpassword == null)){
			$('#LCWnameError').html("内容不能为空");
			$('#LCWpasswordError').html("内容不能为空");
			return false;
		}else{
			$('#LCWnameError').html("");
			$('#LCWpasswordError').html("");
		}
	}
	$.ajax({        			
	   type: "POST",
