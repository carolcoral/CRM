...
load('pagination',firstLoad,'../custom/queryhfStudents.do',1)
...
var tableHeader = "<tr class='henglan'>"+
		...
		"<th class='cjkj'>初级会计实务</th>"+
		"<th class='cjkjScore'>考试成绩</th>"+
		"<th class='jjfjc'>经济法基础</th>"+
		"<th class='jjfjcScore'>考试成绩</th>"+
...
function fillTable(list,currentPage){ 
	$.each(list, function(index, array) {
	    ...
	    var subjectname = array.subjectname|| "";	    
	    var cjkj = array.cjkj|| "";
	    var jjfjc = array.jjfjc|| "";	    
	    var cjkjScore = array.cjkjScore|| "";
	    var jjfjcScore = array.jjfjcScore|| "";		    
        ...
        "<td class='cjkj'>"+cjkj+"</td>"+
        "<td class='cjkjScore'>"+cjkjScore+"</td>"+
        "<td class='jjfjc'>"+jjfjc+"</td>"+
        "<td class='jjfjcScore'>"+jjfjcScore+"</td>"+
...
function fastquery(){
	...
	if(search=="" || search == null){
		...
		load('pagination',firstLoad,'../custom/queryhfStudents.do')//等于空加载所有
	}else {
...
function phoneFill(data,url){
	$.ajax({
		...
		success: function(data) {
			...
			//插入头部
		    $("#resourceTable").append(tableHeader); 
			fillTable(list,currentPage)//插入头部
		}
	})
}
...
function filter(){	
	...
	load('querypagination',filterData,'../custom/queryCustomStudentBySceen.do',1)
}
...
function showdeiatil(){
	...
	if((curret != "" && curret != null) && (filtercurrent == "" || filtercurrent == null)){ //如果当前页不等于空，就重新加载返回curret的列表数据
		$('#querypagination').html('');
		load('pagination',firstLoad,'../custom/queryhfStudents.do')
		...
		$('#pagination').html('');
		load('querypagination',filterData,'../custom/queryCustomStudentBySceen.do')
	}
}
//定时检查页面长度
