	...
	protected String determineBeanNameFromAnnotation(AnnotatedBeanDefinition annotatedDef) {
		...
		for (String type : types) {
			...
			if (isStereotypeWithNameValue(type, amd.getMetaAnnotationTypes(type), attributes)) {
				//Edit by likang,增加判断是否有clazz属性，如果有，则使用此属性的类的全名作为Bean的名字。
				Object clazz = null;
				try {
					clazz = attributes.get("clazz");
				} catch (Exception e) { }
				if(clazz != null){
					if(clazz != Class.class && clazz instanceof Class){
						Class objClass = (Class)clazz;
						beanName = objClass.getName();
					}
				}
				if(clazz == null || beanName == null){
